...
- **Dominant contribution**: QAP-formulated anti-clustering of MoE experts on 3D-NMP, with a direct `max_t T(t)` surrogate (Form A) inside the solver and CoMeT-based verification (Form B) at acceptance, refuted against a same-substrate TACG port.
...
- **C2** TACE reduces peak T against every baseline including TACG-port: 4–7 °C vs random/even-odd, 2–4 °C vs NeuroTAP, 1–3 °C vs Stratum, **2–5 °C vs TACG-port** (paired bootstrap, p < 0.05 with Bonferroni, on held-out family).
...
- **TACG-port closes the gap at low anisotropy** — Mitigation: anisotropy sweep in B8 reports the delta as a function of R_v/R_l; even if magnitude shrinks, sign of TACG-port's thermal harm is the load-bearing finding.
